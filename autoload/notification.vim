...
function! notification#open(msgs)
    try
        call s:notification_create(a:msgs)
    catch
        let s:shown = []
        let s:win_is_open = v:false
        call s:notification_create(a:msgs)
...
function! s:notification_create(msgs)
    if !s:win_is_open && g:notification_sound_cmd != ''
        let timer_id = timer_start(2, function('s:sound_play'), {'repeat' : 1})
    endif

    call s:notification('   ', 'Notification')
    for msg in a:msgs
        call s:notification(msg, 'Notification')
    endfor
endfunction
...
function! s:notification(msg, color) abort
    ...
    if s:win_is_open
        call s:FLOATING.win_config(s:notification_winid,
                    ...
                    \ 'relative': 'editor',
                    \ 'focusable': 0,
                    \ 'width'   : strwidth(a:msg) + 3,
                    \ 'height'  : 1 + len(s:shown),
                    \ 'row': 2,
                    \ 'col': &columns - strwidth(a:msg) - 6
                    \ })
    else
